...
export default function CircleChatPage() {
  ...
  return (
    <div className="h-[calc(100dvh-112px)] xl:h-full lg:min-h-0 w-full flex flex-col xl:flex-row gap-6 items-stretch animate-fadeIn overflow-hidden">
              ...
              {messages.length === 0 ? (
                ...
                messages.map((msg) => {
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`flex gap-2.5 text-left relative group items-end ${isOwn ? 'justify-end' : 'justify-start'}`}
                    >
                      {/* Sender avatar - incoming only */}
                      {!isOwn && (
                        <div className={`w-7.5 h-7.5 rounded-xl flex items-center justify-center font-bold text-[9px] flex-shrink-0 ${msgColorClass} shadow-sm mb-0.5`}>
                          {msgInitials}
                        </div>
                      )}

                      <div className="max-w-[75%] relative flex items-end gap-1">
                        {/* Trash button for owner/admins to moderate, or author to delete own */}
                        {isOwn ? (
                          <button
                            onClick={() => handleDeleteMessage(msg.id)}
                            className="opacity-0 group-hover:opacity-100 p-1 text-stone-400 hover:text-red-500 transition-opacity absolute top-1/2 -left-6 -translate-y-1/2"
                          ...
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 p-1 text-stone-400 hover:text-red-500 transition-opacity absolute top-1/2 -right-6 -translate-y-1/2"
                        ...
                        {/* Bubble */}
                        <div
                          className={`py-2 px-3.5 rounded-[20px] text-xs leading-relaxed shadow-sm font-medium ${
                            isOwn
                              ? 'bg-primary-600 text-white rounded-br-none'
                              : 'bg-stone-50 border border-stone-200/80 text-stone-850 rounded-bl-none'
                          }`}
                        >
                          {/* Sender name inside bubble for incoming */}
                          {!isOwn && (
                            <span className="text-[9px] font-extrabold text-primary-600 block mb-0.5 tracking-wide uppercase select-none">
                              {msg.author_name}
                            </span>
                          )}
                          ...
                          {/* Timestamp and receipts inside bubble */}
                          <div
                            className={`text-[8px] font-bold mt-1 text-right select-none flex items-center justify-end gap-1 ${
                          ...
                          >
                            ...
                            {isOwn && (
                              <span
                                className={`w-1.5 h-1.5 rounded-full border border-stone-200/10 inline-block transition-colors duration-300 ${
                                  msg.viewed_by_others ? 'bg-blue-300' : 'bg-stone-300'
                                }`}
                                title={msg.viewed_by_others ? 'Viewed' : 'Delivered'}
                              />
                            )}
                          </div>
                        </div>
      ...
      {/* Right Column: Interactive Sidebar Widgets (Collaboration Lounge) - Desktop Only */}
      {isMember && (
        <aside className="hidden xl:flex w-full xl:w-[340px] flex-shrink-0 flex-col gap-6 xl:h-full xl:min-h-0 xl:overflow-y-auto xl:overscroll-contain scrollbar-hide pr-1 pb-4">
        ...
        <div className="fixed inset-0 z-50 overflow-hidden flex justify-end animate-fadeIn duration-150">
          ...
          <div className="relative w-full max-w-md bg-stone-50 h-full shadow-2xl flex flex-col justify-between text-left animate-slideInRight duration-200 border-l border-stone-200">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide p-6 space-y-6">
              ...
              {/* Mobile-only additional widgets (Events, Notices, Polls, Member Manager) */}
              <div className="xl:hidden space-y-6 pt-6 border-t border-stone-200">
                <span className="text-[10px] font-bold text-stone-400 uppercase tracking-widest block px-1">
                  Collaboration Hub
                </span>
                ...
                <div className="bg-white border border-stone-200/90 rounded-3xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.02)] text-left space-y-4">
                  ...
                  <div className="space-y-3.5 pr-1">
                    {events.length === 0 ? (
                      ...
                      events.map((ev) => (
                        <div key={ev.id + '-mob'} className="p-3 bg-stone-50/50 border border-stone-150 rounded-2xl text-left space-y-2 relative group">
                          {(ev.created_by === user.id || circle.my_role === 'admin') && (
                            <button
                              onClick={() => handleDeleteEvent(ev.id)}
                              className="absolute bottom-2.5 right-2.5 text-stone-450 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"
                ...
                <div className="bg-white border border-stone-200/90 rounded-3xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.02)] text-left space-y-4">
                  ...
                  <div className="space-y-3 pr-1">
                    {pins.length === 0 ? (
                      ...
                      pins.map((pin) => (
                        <div key={pin.id + '-mob'} className="p-3 bg-yellow-50/50 border border-yellow-100 rounded-2xl relative group">
                          <p className="text-[11px] text-stone-700 font-semibold leading-relaxed pr-6">{pin.content}</p>
                          <span className="text-[8px] font-bold text-stone-400 block mt-1">
                            Pin by {pin.creator_name}
                          </span>
                  ...
                  <div className="space-y-4 pr-1">
                    {polls.length === 0 ? (
                      ...
                      polls.map((p) => {
                        ...
                        return (
                          <div key={p.id + '-mob'} className="p-3 bg-stone-50/50 border border-stone-150 rounded-2xl space-y-3 relative group">
                            {circle.my_role === 'admin' && (
                              <button
                            ...
                            <div>
                              <h4 className="text-xs font-bold text-stone-850 leading-snug">{p.question}</h4>
                              <span className="text-[8px] font-bold text-stone-400 block mt-0.5">📊 {totalVotes} total votes</span>
                            </div>

                            <div className="space-y-2">
                              {p.options.map((o, idx) => {
                                const pct = totalVotes > 0 ? Math.round((o.votes / totalVotes) * 100) : 0
                                const isVoted = o.voted
                                return (
                                  <button
                                    key={idx}
                                    ...
                                    className={`w-full text-left p-2 border rounded-xl relative overflow-hidden transition-all text-[11px] font-semibold flex items-center justify-between ${
                                      isVoted 
                                        ? 'border-primary-350 bg-primary-50/10 text-primary-700 font-bold' 
                                        : 'border-stone-150 bg-white text-stone-600 hover:bg-stone-50/50'
                                    }`}
                                  >
                                    <div 
                                      className="absolute top-0 left-0 bottom-0 bg-primary-100/30 transition-all duration-300 pointer-events-none"
                                      style={{ width: `${pct}%` }}
                                    />
                                    <span className="relative z-10">{o.text}</span>
                                    <span className="relative z-10 text-[9px] font-extrabold text-stone-400">{pct}% ({o.votes})</span>
                                  </button>
                                )
                              })}
                            </div>
                  ...
                  {circle.my_role === 'admin' && (
                    <form onSubmit={handleCreatePoll} className="border-t border-stone-100 pt-3.5 space-y-2.5">
                      ...
                      <input
                        type="text"
                        required
                        placeholder="Question (e.g. Next meeting time?)"
                        value={pollQuestion}
                        onChange={(e) => setPollQuestion(e.target.value)}
                        className="w-full h-8 px-2.5 border border-stone-200 rounded-lg text-[10px] font-medium"
                      />
                ...
                <div className="bg-white border border-stone-200/90 rounded-3xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.02)] text-left space-y-4">
                  ...
                  {circle.my_role === 'admin' && joinRequests.length > 0 && (
                    <div className="space-y-2">
                      ...
                      <div className="space-y-2 max-h-[140px] overflow-y-auto pr-1">
                        {joinRequests.map((req) => (
                          <div key={req.user_id + '-mob'} className="p-2.5 bg-stone-50 border border-stone-150 rounded-xl flex items-center justify-between gap-3 text-xs">
                            <div className="min-w-0">
                              <p className="font-bold text-stone-800 truncate">{req.name}</p>
                              <p className="text-[9px] text-stone-400 truncate">{req.email}</p>
                            </div>
